...
func TestCannotCommitWithoutNewTermEntry(t *testing.T) {
	...
	if sm.raftLog.committed != 1 {
		t.Errorf("committed = %d, want %d", sm.raftLog.committed, 0)
	}
...
func TestDuelingCandidates(t *testing.T) {
	...
	wlog := &raftLog{ents: []Entry{{}, {Type: Normal, Data: nil, Term: 1, Index: 1}}, committed: 1}
...
func TestCommit(t *testing.T) {
	tests := []struct {
		...
		{[]int64{2}, []Entry{{}, {Term: 1}, {Term: 2}}, 1, 0},
	...
	for i, tt := range tests {
		...
		sm := &stateMachine{raftLog: &raftLog{ents: tt.logs}, indexs: ins, term: atomicInt(tt.smTerm)}
		sm.maybeCommit()
	...
	for i, tt := range tests {
		...
		for j, msgType := range tmsgTypes {
			...
			if sm.lead.Get() != wlead {
				t.Errorf("#%d.%d lead = %v , want %v", i, j, sm.lead, wlead)
			}
		}
	}
}
...
func TestProvideSnap(t *testing.T) {
	...
	sm.indexs[1].next = sm.raftLog.offset
...
func newNetwork(peers ...Interface) *network {
	...
	for id, p := range peers {
		...
		case *stateMachine:
			v.id = nid
			v.indexs = make(map[int64]*index)
			for i := 0; i < size; i++ {
				v.indexs[int64(i)] = &index{}
			}
			v.reset(0)
			npeers[nid] = v
		case *Node:
...
func (nw *network) cut(from, to int64) {
	nw.drop(from, to, 1)
	nw.drop(to, from, 1)
}

// cut the connection between all nodes
