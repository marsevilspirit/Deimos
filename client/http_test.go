...
func TestURLHelper(t *testing.T) {
	tests := []struct {
		...
		// key is empty, no problem
		{
			...
			key:      "",
			want:     url.URL{Scheme: "http", Host: "example.com", Path: "/keys"},
		},
		...
		// key is joined to path
		{
			endpoint: url.URL{Scheme: "http", Host: "example.com", Path: ""},
			key:      "/foo/bar",
			want:     url.URL{Scheme: "http", Host: "example.com", Path: "/keys/foo/bar"},
		},
		...
		// Host field carries through with port
		{
			...
			key:      "",
			want:     url.URL{Scheme: "http", Host: "example.com:8080", Path: "/keys"},
		},
		...
		// Scheme carries through
		{
			...
			key:      "",
			want:     url.URL{Scheme: "https", Host: "example.com", Path: "/keys"},
		},
		...
		// Path on endpoint is not ignored
		{
			...
			key:      "/foo",
			want:     url.URL{Scheme: "https", Host: "example.com", Path: "/prefix/keys/foo"},
		},
	}

	for i, tt := range tests {
		got := URL(tt.endpoint, tt.key)
		if tt.want != *got {
