...
func TestDoLocalAction(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			Response{Watcher: &stubWatcher{}}, nil, []action{{name: "Watch"}},
		},
		{
	...
	tests := []struct {
		...
		// POST ==> Create
		{
			...
			[]action{
				{
					name:   "Create",
					params: []any{"", false, "", true, time.Time{}},
				},
			},
		},
		// POST ==> Create, with expiration
		{
			...
			[]action{
				{
					name:   "Create",
					params: []any{"", false, "", true, time.Unix(0, 1337)},
				},
			},
		},
		// POST ==> Create, with dir
		{
			...
			[]action{
				{
					name:   "Create",
					params: []any{"", true, "", true, time.Time{}},
				},
			},
		},
		// PUT ==> Set
		...
		// PUT with PrevExist=false ==> Create
		{
			...
			[]action{
				{
					name:   "Create",
					params: []any{"", false, "", false, time.Time{}},
				},
			},
		},
		...
		// PUT with PrevValue set ==> CompareAndSwap
		{
			...
			[]action{
				{
					name:   "CompareAndSwap",
					params: []any{"", "bar", uint64(0), "", time.Time{}},
				},
			},
		},
		// PUT with PrevIndex and PrevValue set ==> CompareAndSwap
		{
			...
			[]action{
				{
					name:   "CompareAndSwap",
					params: []any{"", "bar", uint64(1), "", time.Time{}},
				},
			},
		},
		// DELETE ==> Delete
...
func TestDoProposalCancelled(t *testing.T) {
	...
	w := []action{{name: "Register1"}, {name: "Trigger1"}}
...
func TestRecvSnapshot(t *testing.T) {
	...
	wactions := []action{{name: "Recovery"}}
	if g := st.Action(); !reflect.DeepEqual(g, wactions) {
		t.Errorf("store action = %v, want %v", g, wactions)
	}
	wactions = []action{{name: "Save"}, {name: "SaveSnap"}}
...
func TestAddNode(t *testing.T) {
	...
	wactions := []action{{name: "ProposeConfChange:ConfChangeAddNode"}, {name: "ApplyConfChange:ConfChangeAddNode"}}
...
func TestRemoveNode(t *testing.T) {
	...
	wactions := []action{{name: "ProposeConfChange:ConfChangeRemoveNode"}, {name: "ApplyConfChange:ConfChangeRemoveNode"}}
	if !reflect.DeepEqual(gaction, wactions) {
		t.Errorf("action = %v, want %v", gaction, wactions)
	}
}
...
func (s *storeRecorder) Create(path string, dir bool, val string, uniq bool, exp time.Time) (*store.Event, error) {
	s.record(action{
		name:   "Create",
		params: []any{path, dir, val, uniq, exp},
	})
	return &store.Event{}, nil
}
func (s *storeRecorder) CompareAndSwap(path, prevVal string, prevIdx uint64, val string, expr time.Time) (*store.Event, error) {
	s.record(action{
		name:   "CompareAndSwap",
		params: []any{path, prevVal, prevIdx, val, expr},
	})
	return &store.Event{}, nil
}
